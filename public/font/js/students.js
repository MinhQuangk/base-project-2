...
function studentFunc(evt, studentName) {
  ...
  s_tabcontent = document.getElementsByClassName("tab {{ !session('student_tabcontent') ? 'active', null }}");
  for (i = 0; i < s_tabcontent.length; i++) {
      s_tabcontent[i].style.display = "none";
  }
  s_tablinks = document.getElementsByClassName("tab {{ !session('student_tabcontent') ? 'active', null }}");
